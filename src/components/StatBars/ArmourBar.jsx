import armourIcon from '/src/assets/armour.png'
...
function TickRow({ fill, rowIndex, groupIndex }) {
  ...
  const activeClass = `${styles.tick} ${styles.armour} ${active ? styles.active : ''}`
...
export default function CyberwareBar({ className, now, label }) {
  ...
  return (
    <div className={`${className} ${styles['stat-bar']}`}>
      <div
        className={`${styles.label} ${styles.armour}`}
        style={{ bottom: `${now - 6}%` }}
      >
        {label}
      </div>
